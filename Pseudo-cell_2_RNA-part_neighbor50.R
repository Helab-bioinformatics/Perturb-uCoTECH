# =============================================================================================
# R codes provided by Chen Li in August, 2023.
# =============================================================================================
...
feature.gene <- rownames(triomics.rds.perturb@assays$RNA@data)

sum(colnames(cn) != cell.name)

# calculate RNA signals in enhancers --------------
num.feat <- 1
RNA.mean <- apply(sgRNAonCRE.mat, MARGIN = 1, FUN = function(x){
    num.cell <- 1
    tmp <- sapply(x, FUN = function(y) {
      ...
      if(exists("neighbor1") & pair[feature.name[num.feat], "gene"] %in% feature.gene) {trueM <- triomics.rds.perturb@assays$RNA@data[pair[feature.name[num.feat],'gene'], neighbor1] %>% mean()} else {trueM <-0}
      if(exists("neighbor0") & pair[feature.name[num.feat], "gene"] %in% feature.gene) {falseM <- triomics.rds.perturb@assays$RNA@data[pair[feature.name[num.feat], "gene"], neighbor0] %>% mean()} else {falseM <-0}
...
RNA.mean <- RNA.mean[!sapply(RNA.mean, FUN = function(x){length(x)==0})]
summary <- sapply(RNA.mean, FUN = function(x){
  RNA.mean.unls <- unlist(x)
  k27.enh.df <- data.frame(trueM = RNA.mean.unls[seq(1,length(RNA.mean.unls), 2)], falseM=RNA.mean.unls[seq(2,length(RNA.mean.unls), 2)])
...
summary <- t(summary)
write.csv(summary, "Pseudo-cell_RNA-part.csv", quote = F)
